...
const create = (() => {
  // make temp toggle
  const temp = () => {
    const tempBtnContainer = createElement("div", {
      class: "tempBtnContainer",
    });
  ...
  // on mobile menu - remove italics from user's previous selection
  const resetFont = (array) => {
    array.forEach((index) => {
      index.style.fontStyle = "normal";
    });
  };
  // mobile menu functionality -> display selection, hide other content
  ...
  // create menu for mobile (button in header)
  const menu = () => {
  ...
  // build initial page (header, blank main div, footer)
  const initial = () => {
...
const fillSearchContainer = () => {
  ...
  menuList.children[0].children[1].addEventListener("click", (e) => {
    create.listFn(e, menuList, main.children[1], "flex", [
      main.children[0],
      main.children[2],
    ]);
  });
  ...
  menuList.children[0].children[2].addEventListener("click", (e) => {
    create.listFn(e, menuList, main.children[0], "grid", [
      main.children[1],
      main.children[2],
    ]);
  });
  ...
  menuList.children[0].children[3].addEventListener("click", (e) => {
    create.listFn(e, menuList, main.children[2], "grid", [
      main.children[0],
      main.children[1],
    ]);
  });
